...
// Get all leads with pagination (Admin and Manager only)
...
// Get leads assigned to current user with pagination (Manager and Staff only)
export const getMyLeadsPaginated = query({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      console.error("getMyLeadsPaginated error:", err);
      return [];
    }
  },
});
